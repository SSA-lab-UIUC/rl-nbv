...
def caculate_average_coverage(env, model, step_size, output_file, logger):
    ...
    logger.info(
        "Calculating average coverage [{}] over {} models...".format(label, model_size)
    )
...
def compare_and_report(model_cov, random_cov, step_size, output_file, logger):
    ...
    logger.info(
        "Final step  |  Model: {:.2f}%  |  Random: {:.2f}%  |  Advantage: {:+.2f}%".format(
...
if __name__ == "__main__":
    ...
    if advantage > 0:
        logger.info(
            "✅ Trained model outperforms random by {:.2f}% at final step.".format(
                advantage
            )
        )
    else:
        logger.warning(
            "⚠️  Random policy matches or beats trained model by {:.2f}%.".format(
                abs(advantage)
            )
        )
